...
1. **Обробка вхідного повідомлення**
   ...
   - Сервіс: `speech-service` (speech-service)
     - Локальна Whisper модель для STT
     - Оптимізація аудіопотоку
     - Кешування частих фраз
     - Мультимовна підтримка
     - Конфігурація параметрів моделі для балансу швидкості/якості
...
- OpenAI API ключ
- Мінімум 8 ГБ RAM
- NVIDIA GPU з CUDA підтримкою (опціонально, для прискорення STT)
...
- `WHISPER_MODEL_SIZE`: Виберіть `base`, `tiny`, `small`, `medium` або `large`
- `WHISPER_DEVICE`: Виберіть `cuda` або `cpu`
- `WHISPER_HOST`: URL для сервісу розпізнавання мовлення
- `WHISPER_PORT`: Порт для сервісу розпізнавання мовлення
- `AUDIO_CACHE_ENABLED`: Увімкніть кешування аудіо (true/false)
- `AUDIO_CACHE_TTL`: Час життя кеша в секундах (за замовчуванням 3600)
- `STT_SUPPORTED_LANGUAGES`: Список мов, які підтримує STT
- `WHISPER_BATCH_SIZE`: Розмір пакету для STT (за замовчуванням 16)
- `WHISPER_COMPUTE_TYPE`: Тип обчислень для STT (за замовчуванням float16)
...
- `VECTOR_STORE_PORT`: Порт для векторної бази даних (за замовчуванням 8082)
